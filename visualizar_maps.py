...
def imprimir_inferencia(serie=0, original=True):
    ...
    seg_out2 = nib.load(label_add2).get_fdata()
    seg_out2 = seg_out2.squeeze()
    ...
    def update_slice(slice_num):
        ...
        ax[0, 1].imshow(
            np.rot90(seg_out2[:, :, slice_num], k=-1), cmap="jet", alpha=0.3
        )
        ax[0, 1].set_title("froi map")
...
def main():
    ...
    imprimir_inferencia(args.serie, original=args.o)
